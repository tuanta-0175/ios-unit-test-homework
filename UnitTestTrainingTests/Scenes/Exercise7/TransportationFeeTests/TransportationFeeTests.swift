...
final class TransportationFeeTests: XCTestCase, CaculatingTransportationFee {
    override func setUp() {
        super.setUp()
    }
    ...
    func testCase2_calculationFee() {
        let dto = VietnamMartOrderDto(cartAmount: "5200.0", isPremiumMember: true, isQuickDeliver: false)
        let result = self.calculationFee(dto: dto)
        XCTAssertEqual(result.standardFee, 0.0)
    ...
    func testCase3_calculationFee() {
        let dto = VietnamMartOrderDto(cartAmount: "4800.0", isPremiumMember: true, isQuickDeliver: true)
        let result = self.calculationFee(dto: dto)
        XCTAssertEqual(result.standardFee, 0.0)
        XCTAssertEqual(result.quickFee, 500.0)
    }
    ...
    func testCase5_calculationFee() {
        let dto = VietnamMartOrderDto(cartAmount: "8000.0", isPremiumMember: false, isQuickDeliver: true)
        let result = self.calculationFee(dto: dto)
        XCTAssertEqual(result.standardFee, 0.0)
        XCTAssertEqual(result.quickFee, 500.0)
    }
    ...
    func testCase6_calculationFee() {
        let dto = VietnamMartOrderDto(cartAmount: "5200.0", isPremiumMember: false, isQuickDeliver: false)
        let result = self.calculationFee(dto: dto)
        XCTAssertEqual(result.standardFee, 0.0)
        XCTAssertEqual(result.quickFee, 0.0)
    }
    ...
    func test_validateCardAmount_success() {
        let result = self.validateCardAmount("2500")
    ...
    func test_validateCardAmount_failed_NaN() {
        let result = self.validateCardAmount("abcd")
        XCTAssertFalse(result.isValid)
    }
}
